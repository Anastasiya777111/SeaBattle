...
init() {
    this.status = document.querySelector(".fieldOfBattle-status");
}
    ...
    update() {
        ...
        cells.forEach(cell=>cell.classList.remove('fieldOfBattle-item__active'));
        ...
        if (isUnderPoint(mouse, opponent.table)){
            ...
            if (cell) {
				cell.classList.add("fieldOfBattle-item__active");
		...
		if (!this.playerTurn) {
			const x = getMinMax(0, 9);
			const y = getMinMax(0, 9);
